<template>
  <div>
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h5 class="fw-bold mb-0"><i class="bi bi-building me-2 text-primary"></i>長照機構管理</h5>
      <button v-if="isAdmin" class="btn btn-primary btn-sm" @click="openNew">
        <i class="bi bi-plus-lg me-1"></i>新增機構
      </button>
    </div>
          ...
          <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
              ...
              <tr v-for="u in units" :key="u.id">
                ...
                <td class="text-muted small">{{ u.contact_person ?? u.contactPerson ?? '—' }}</td>
                <td class="text-muted small">{{ u.phone ?? '—' }}</td>
                <td class="text-muted small">{{ u.address ?? '—' }}</td>
                <td v-if="isAdmin">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h6 class="modal-title fw-bold">{{ editId ? '編輯機構' : '新增機構' }}</h6>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body">
            <div class="row g-3">
              <div class="col-12">
                <label class="form-label small fw-semibold">機構名稱 *</label>
...
import { careUnitsApi } from '@/api/index.js'
...
async function loadData() {
  loading.value = true
  try {
    const res = await careUnitsApi.list()
    const d = res.data
    units.value = Array.isArray(d) ? d : (d.data ?? [])
  } finally {
    loading.value = false
  }
}
...
function openEdit(u) {
  ...
  form.value = {
    name: u.name ?? '',
    contact_person: u.contact_person ?? u.contactPerson ?? '',
    phone: u.phone ?? '',
    address: u.address ?? '',
    notes: u.notes ?? ''
  }
...
async function remove(u) {
  if (!confirm(`確定要刪除「${u.name}」？`)) return
  try {
    await careUnitsApi.delete(u.id)
    await loadData()
  } catch (err) {
...
async function submit() {
  ...
  try {
    if (editId.value) {
      await careUnitsApi.update(editId.value, form.value)
    } else {
      await careUnitsApi.create(form.value)
    }
    bsModal?.hide()
    await loadData()
  } catch (err) {
